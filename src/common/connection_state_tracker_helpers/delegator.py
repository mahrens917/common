...
class StateUpdaterDelegator:
    ...
    async def update_connection_state(
        ...
        try:
            ...
            return await self.tracker.state_updater.update_connection_state(
...
class StateQuerierDelegator:
    ...
    async def is_service_in_reconnection(self, service_name: str) -> bool:
        ...
        try:
            ...
            raise build_tracker_error(
                f"Failed to determine reconnection status for {service_name}", exc
            )
    ...
    async def is_service_in_grace_period(
        ...
        await self.tracker.initialize()
        assert self.tracker.state_querier is not None
        return await self.tracker.state_querier.is_service_in_grace_period(
    ...
    async def get_reconnection_duration(self, service_name: str) -> Optional[float]:
        ...
        try:
            ...
            raise build_tracker_error(
                f"Failed to compute reconnection duration for {service_name}", exc
            )
...
class EventManagerDelegator:
    ...
    async def record_connection_event(
        ...
        try:
            await self.tracker.event_manager.record_connection_event(
    ...
    async def get_recent_connection_events(
        ...
        assert self.tracker.event_manager is not None
        try:
            return await self.tracker.event_manager.get_recent_connection_events(
