...
class RelationshipValidator:
    ...
    @staticmethod
    def validate_spread_relationship(
        best_bid: float, best_ask: float, absolute_spread: float
    ) -> None:
        ...
        if abs(absolute_spread - expected_abs_spread) > _NUMERICAL_TOLERANCE:
            raise ValueError(
                f"Absolute spread ({absolute_spread}) must equal best_ask - best_bid ({expected_abs_spread})"
            )

    @staticmethod
    def validate_relative_spread(
        absolute_spread: float, relative_spread: float, p_raw: float
    ) -> None:
        ...
        if abs(relative_spread - expected_rel_spread) > _NUMERICAL_TOLERANCE:
            raise ValueError(
                f"Relative spread ({relative_spread}) must equal absolute_spread / p_raw ({expected_rel_spread})"
            )

    @staticmethod
    def validate_intensity_calculation(
        best_bid_size: float, best_ask_size: float, i_raw: float
    ) -> None:
        ...
        if total_volume > 0:
            ...
            if abs(i_raw - expected_i_raw) > _NUMERICAL_TOLERANCE:
                raise ValueError(
                    f"I_raw ({i_raw}) must equal best_bid_size / (best_bid_size + best_ask_size) ({expected_i_raw})"
                )
    ...
    def validate_micro_price_calculation(
        ...
        if total_volume > 0:
            ...
            if abs(p_raw - expected_p_raw) > _NUMERICAL_TOLERANCE:
                raise ValueError(
                    f"p_raw ({p_raw}) must equal volume-weighted micro price ({expected_p_raw})"
                )
