...
def validate_g_transformation(absolute_spread: float, g: float) -> None:
    ...
    if absolute_spread > 0:
        ...
        if relative_error > NUMERICAL_TOLERANCE:
            raise ValueError(
                f"g ({g}) must equal log(absolute_spread) ({expected_g}) within tolerance {NUMERICAL_TOLERANCE}, got relative error {relative_error}"
            )
...
def validate_h_transformation(i_raw: float, h: float) -> None:
    ...
    if 0 < i_raw < 1:
        ...
        if relative_error > NUMERICAL_TOLERANCE:
            raise ValueError(
                f"h ({h}) must equal log(I_raw / (1 - I_raw)) ({expected_h}) within tolerance {NUMERICAL_TOLERANCE}, got relative error {relative_error}"
            )
