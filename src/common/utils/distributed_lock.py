...
class DistributedLock:
    ...
    async def acquire(self) -> bool:
        ...
        if not self.redis_client:
            raise LockUnavailableError(f"Redis client is required to acquire distributed lock '{self.lock_key}'")
        ...
        if not lock_acquired:
            raise LockUnavailableError(f"Distributed lock '{self.lock_key}' is already held by another process")
...
async def create_trade_lock(redis_client, trade_id: str, timeout_seconds: int = 30) -> DistributedLock:
...
async def create_liquidation_lock(redis_client, position_id: str, timeout_seconds: int = 60) -> DistributedLock:
