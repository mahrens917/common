...
class WeatherFetcher:
    ...
    async def fetch_weather_markets(self, category: str, markets: List[Dict[str, object]], seen_tickers: set[str]) -> int:
        ...
        if matched_series == 0:
            raise KalshiMarketCatalogError(f"Weather series for {category} returned no KXHIGH tickers")
        return pages
