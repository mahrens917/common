...
class ServiceStateCollector:
    ...
    async def collect_running_services(self) -> List[Dict[str, str]]:
        ...
        for service_name in self.process_manager.services:
            if self._check_service_status(
                service_name, PidValidator, ProcessRediscoverer, ServiceInfoUpdater
            ):
    ...
    def _check_service_status(
        ...
        if info and not isinstance(info.status, ProcessStatus):
            raise TypeError(
                f"Service {service_name} has invalid status type {type(info.status)}: {info.status}. "
                "Expected ProcessStatus enum."
            )
        is_running = self._check_process_handle(service_name, info, updater)
        if is_running:
            return True
        is_running, info = rediscoverer.rediscover_and_validate(
            service_name, self.process_manager, pid_validator
        )
    ...
    def _check_process_handle(self, service_name: str, info, updater) -> bool:
        process_handle = (
            self.process_manager.processes.get(service_name)
            if hasattr(self.process_manager, "processes")
            else None
        )
