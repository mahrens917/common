...
def build_result_dict(core_components: dict, components: ResultComponents) -> dict:
...
def create_services(
    ...
    *,
    initialized_weather: Any,
    telegram_handler: Any,
    services_holder: dict,
) -> dict:
    """Create and wire service stack from core components."""
    portfolio, orders, trade_collection = _create_service_stack(
        core_components["kalshi_client"],
        core_components["trade_store_manager"],
        core_components["notifier"],
        initialized_weather,
        services_holder,
        telegram_handler,
    )
    _wire_order_dependencies(orders, core_components["notifier"], telegram_handler)
    return {
        "portfolio": portfolio,
        "orders": orders,
...
def create_service_providers(trade_store_manager: Any, services_holder: dict) -> dict:
    """Create service provider functions."""

    def get_trade_store() -> Any:
        return trade_store_manager.get_or_create()

    def get_order_poller() -> Any:
