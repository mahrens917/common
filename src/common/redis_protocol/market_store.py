...
class DeribitStore(OptimizedMarketStore):
    ...
    def __init__(self, redis: Any | None = None):
        if redis is None:
            raise TypeError("DeribitStore requires an explicit Redis client; use OptimizedMarketStore.create() for pool-based usage.")
    ...
    async def get_usdc_bid_ask_prices(self, currency: str) -> Tuple[float, float]:
        ...
        if self.atomic_ops:
            market_data = await self.atomic_ops.safe_market_data_read(pair_key, required_fields=["best_bid", "best_ask"])
        else:
            market_data = await redis.hgetall(pair_key)

        if not market_data:
            raise DataError(f"USDC pair market data not available for {currency}; key '{pair_key}' missing")

        bid_str = market_data.get("best_bid")
        ask_str = market_data.get("best_ask")
        if bid_str is None or ask_str is None:
            raise DataError(f"USDC pair market data incomplete for {currency}; bid='{bid_str}', ask='{ask_str}'")
    ...
    async def get_usdc_micro_price(self, currency: str) -> float:
        ...
        if self.atomic_ops:
            market_data = await self.atomic_ops.safe_market_data_read(pair_key, required_fields=required_fields)
        else:
            market_data = await redis.hgetall(pair_key)
