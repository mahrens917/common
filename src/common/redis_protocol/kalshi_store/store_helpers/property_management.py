...
def create_mgr_property(attr: str):
    ...
    def getter(s):
        ...
        return (
            object.__getattribute__(s, f"_{attr}_value") if hasattr(s, f"_{attr}_value") else None
        )
...
def setup_kalshi_store_properties(store_class) -> None:
    ...
    store_class.SERVICE_STATUS_KEY = property(
        lambda s: s._subscription_delegator.SERVICE_STATUS_KEY
    )
    store_class.SUBSCRIBED_MARKETS_KEY = property(
        lambda s: s._subscription_delegator.SUBSCRIBED_MARKETS_KEY
    )
    store_class.SUBSCRIPTION_IDS_KEY = property(
        lambda s: s._subscription_delegator.SUBSCRIPTION_IDS_KEY
    )
