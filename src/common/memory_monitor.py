...
class MemoryMonitor:
    def __init__(self, service_name: str, check_interval_seconds: int = 60):
        ...
        (self._metrics_reader, self._collection_tracker, self._snapshot_collector,
         self._trend_analyzer, self._alert_logger, self._loop_manager,
         self._status_formatter) = MemoryMonitorFactory.create_components(
            service_name, check_interval_seconds, self._memory_growth_threshold_mb,
            self._collection_growth_threshold, self._task_count_threshold, self.max_snapshots)
