...
def test_sequence_and_mapping_delegations(monkeypatch):
    monkeypatch.setattr(value_coercion.utils_coercion, "coerce_sequence", lambda candidate: list(candidate))
    monkeypatch.setattr(value_coercion.utils_coercion, "coerce_mapping", lambda candidate: dict(candidate))
...
def test_numeric_and_int_coercions(monkeypatch):
    monkeypatch.setattr(value_coercion.utils_coercion, "convert_numeric_field", lambda value: float(value))
    monkeypatch.setattr(
        value_coercion.utils_coercion,
        "float_or_default",
        lambda value, otherwise_val, **kwargs: float(value),
    )
    monkeypatch.setattr(value_coercion.utils_coercion, "int_or_default", lambda value, otherwise_val: int(value))
