...
async def test_log_activity_classifies_stale(tmp_path, monkeypatch):
    monkeypatch.setattr(
        ...
        lambda key, or_value=None, **kwargs: {"LOG_RECENT_THRESHOLD": 60, "LOG_STALE_THRESHOLD": 900}.get(key, or_value),
    )
...
async def test_log_activity_classifies_old(tmp_path, monkeypatch):
    monkeypatch.setattr(
        "common.health.log_activity_monitor.env_int",
        lambda key, or_value=None, **kwargs: {"LOG_RECENT_THRESHOLD": 60, "LOG_STALE_THRESHOLD": 600}.get(key, or_value),
    )
...
async def test_get_all_service_log_activity(tmp_path, monkeypatch):
    monkeypatch.setattr(
        "common.health.log_activity_monitor.env_int",
        lambda key, or_value=None, **kwargs: {"LOG_RECENT_THRESHOLD": 10, "LOG_STALE_THRESHOLD": 60}.get(key, or_value),
    )
