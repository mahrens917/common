...
from common.redis_protocol.connection import (
    RedisConnection,
...
async def test_redis_connection_connect_establishes_connection():
    ...
    with (
        ...
        patch("common.redis_protocol.connection.record_pool_acquired") as mock_record,
    ):
...
async def test_redis_connection_close_closes_client():
    ...
    with (
        ...
        patch("common.redis_protocol.connection.record_pool_returned") as mock_record,
    ):
        conn = RedisConnection()
        await conn.connect()
        await conn.close()
...
async def test_redis_connection_close_handles_redis_error():
    ...
    with (
        ...
        patch("common.redis_protocol.connection.logger"),
    ):
        conn = RedisConnection()
        await conn.connect()
        await conn.close()
