...
def test_resolve_market_strike_returns_calculated_midpoint():
    ...
    result = resolve_market_strike(metadata, _string_converter)
...
def test_resolve_market_strike_falls_back_to_floor_or_cap():
    ...
    assert resolve_market_strike(floor_only, _string_converter) == pytest.approx(5.0)
    ...
    assert resolve_market_strike(cap_only, _string_converter) == pytest.approx(8.0)


def test_resolve_market_strike_returns_none_without_type():
    assert resolve_market_strike({}, _string_converter) is None
