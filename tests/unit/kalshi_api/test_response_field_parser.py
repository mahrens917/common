...
from common.kalshi_api.response_field_parser import (
    extract_raw_values,
    extract_ticker,
    normalise_fill,
...
def test_extract_raw_values_success():
    ...
    result = extract_raw_values(payload)
...
def test_parse_fills_list_item_not_dict():
    with pytest.raises(KalshiClientError) as exc_info:
        parse_fills_list(["not a dict"])

    assert "must be a JSON object" in str(exc_info.value)


def test_extract_ticker_success():
    result = extract_ticker({"ticker": "  ABC-DEF  "})
    assert result == "ABC-DEF"


def test_extract_ticker_empty():
    with pytest.raises(KalshiClientError) as exc_info:
        extract_ticker({"ticker": "   "})
