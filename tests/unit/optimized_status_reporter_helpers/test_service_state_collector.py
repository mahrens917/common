...
class TestServiceStateCollector:
    ...
    def test_check_service_status_process_handle_running(
        ...
        is_running = collector._check_service_status(
            "service_A", mock_pid_validator, mock_rediscoverer, mock_updater
        )

        assert is_running is True
        mock_check_process_handle.assert_called_once_with(
            "service_A", mock_process_manager.process_info["service_A"], mock_updater
        )
    ...
    def test_check_service_status_rediscoverer_finds_process(
        ...
        is_running = collector._check_service_status(
            "service_B", mock_pid_validator, mock_rediscoverer, mock_updater
        )
    ...
    def test_check_process_handle_process_not_running(
        ...
        mock_process_manager.processes["service_A"].poll.return_value = (
            1  # Not None, so process is stopped
        )
