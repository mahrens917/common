...
class TestServiceStateCollector:
    ...
    async def test_collect_running_services(self, collector):
        ...
        with patch.object(collector, "_check_service_status") as mock_check_service_status:
            ...
            mock_check_service_status.assert_any_call("service_A")
            mock_check_service_status.assert_any_call("service_B")
    ...
    def test_check_service_status_invalid_status_type(self, collector, mock_process_manager):
        ...
        with pytest.raises(TypeError, match="invalid status type"):
            collector._check_service_status("service_A")

    @patch(f"{_HELPERS_PATH}.mark_as_running")
    @patch(f"{_HELPERS_PATH}.clear_stopped_process")
    @patch(f"{_HELPERS_PATH}.rediscover_and_validate")
    ...
    def test_check_service_status_process_handle_running(
        ...
        is_running = collector._check_service_status("service_A")

        assert is_running is True
        mock_check_process_handle.assert_called_once_with("service_A", mock_process_manager.process_info["service_A"])
    ...
    def test_check_service_status_rediscoverer_finds_process(
        ...
        mock_rediscover.return_value = (
            True,
            Mock(),
        )  # (is_running, info)

        is_running = collector._check_service_status("service_B")
    ...
    def test_check_service_status_rediscoverer_fails_no_info(
        ...
        mock_check_process_handle.return_value = False
        mock_rediscover.return_value = (False, None)  # (is_running, info)
    ...
    @patch(f"{_HELPERS_PATH}.update_from_handle")
    def test_check_process_handle_no_process_handle(self, mock_update, collector, mock_process_manager):
        """Test _check_process_handle returns False if no process handle."""
        mock_process_manager.processes = {}  # No processes defined

        is_running = collector._check_process_handle("service_A", Mock(pid=123))
        assert is_running is False
        mock_update.assert_not_called()

    @patch(f"{_HELPERS_PATH}.update_from_handle")
    def test_check_process_handle_process_not_running(self, mock_update, collector, mock_process_manager):
    ...
    @patch(f"{_HELPERS_PATH}.update_from_handle")
    def test_check_process_handle_process_running_updates_info(self, mock_update, collector, mock_process_manager):
        ...
        is_running = collector._check_process_handle("service_A", mock_info)
        assert is_running is True
        mock_update.assert_called_once_with(mock_info, mock_process_handle)
