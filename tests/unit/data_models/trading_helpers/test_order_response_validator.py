...
class TestOrderResponseValidator(unittest.TestCase):
    def test_validate_order_response_enums_success(self):
        validate_order_response_enums(OrderStatus.FILLED, OrderSide.YES, OrderAction.BUY, OrderType.LIMIT)
    ...
    def test_validate_order_response_enums_failure(self):
        ...
        with self.assertRaises(TypeError):
            validate_order_response_enums(OrderStatus.FILLED, "YES", OrderAction.BUY, OrderType.LIMIT)

        with self.assertRaises(TypeError):
            validate_order_response_enums(OrderStatus.FILLED, OrderSide.YES, "BUY", OrderType.LIMIT)

        with self.assertRaises(TypeError):
            validate_order_response_enums(OrderStatus.FILLED, OrderSide.YES, OrderAction.BUY, "LIMIT")
    ...
    def test_validate_order_response_metadata_success(self):
        validate_order_response_metadata("order_id", "client_id", "ticker", "rule", "reason longer than 10", datetime.now())

    def test_validate_order_response_metadata_failure(self):
        with self.assertRaises(ValueError):
            validate_order_response_metadata("", "client_id", "ticker", "rule", "reason", datetime.now())

        with self.assertRaises(ValueError):
            validate_order_response_metadata("order_id", "", "ticker", "rule", "reason", datetime.now())

        with self.assertRaises(ValueError):
            validate_order_response_metadata("order_id", "client_id", "", "rule", "reason", datetime.now())

        with self.assertRaises(TypeError):
            validate_order_response_metadata("order_id", "client_id", "ticker", "rule", "reason", "not datetime")

        with self.assertRaises(ValueError):
            validate_order_response_metadata("order_id", "client_id", "ticker", "", "reason", datetime.now())

        with self.assertRaises(ValueError):
            validate_order_response_metadata("order_id", "client_id", "ticker", "rule", "", datetime.now())

        with self.assertRaises(ValueError):
            validate_order_response_metadata("order_id", "client_id", "ticker", "rule", "short", datetime.now())
