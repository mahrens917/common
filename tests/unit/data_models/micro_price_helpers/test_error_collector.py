...
from common.data_models.micro_price_helpers.error_collector import (
    collect_basic_data_errors,
    collect_intensity_errors,
    collect_micro_price_errors,
    collect_reconstruction_errors,
    collect_spread_errors,
...
class TestCollectMicroPriceErrors:
    ...
    def test_error_for_price_above_ask(self) -> None:
        ...
        errors = collect_micro_price_errors(0.05, 0.10, 0.12)

        assert len(errors) == 1
        assert "Micro price constraint violated" in errors[0]


class TestCollectReconstructionErrors:
    """Tests for collect_reconstruction_errors."""
    ...
    def test_no_errors_for_valid_reconstruction(self) -> None:
        ...
        # 0.075 + 0.5 * 0.05 = 0.10
        errors = collect_reconstruction_errors(
            best_bid=0.05,
            best_ask=0.10,
            p_raw=0.075,
            i_raw=0.5,
...
class TestCollectBasicDataErrors:
    """Tests for collect_basic_data_errors."""

    def test_no_errors_for_valid_data(self) -> None:
        """Returns empty list for valid data."""
        errors = collect_basic_data_errors(10.0, 20.0, "call")

        assert errors == []

    def test_no_errors_for_put_option(self) -> None:
        """Returns empty list for put option."""
        errors = collect_basic_data_errors(10.0, 20.0, "put")
    ...
    def test_error_for_invalid_option_type(self) -> None:
        ...
        errors = collect_basic_data_errors(10.0, 20.0, "straddle")
...
class TestGetValidationErrors:
    """Tests for get_validation_errors."""
    ...
    def test_returns_empty_list_for_valid_params(self) -> None:
        ...
        errors = get_validation_errors(params)
