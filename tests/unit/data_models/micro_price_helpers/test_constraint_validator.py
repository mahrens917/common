"""Unit tests for constraint_validator module."""

from __future__ import annotations

import pytest

from common.data_models.micro_price_helpers.constraint_validator import (
    validate_ask_reconstruction,
    validate_bid_reconstruction,
    validate_intensity_constraint,
    validate_micro_price_bounds,
    validate_micro_price_constraints,
    validate_spread_constraint,
)
...
class TestValidateSpreadConstraint:
    ...
    def test_accepts_zero(self) -> None:
        validate_spread_constraint(0.0)

    def test_accepts_positive(self) -> None:
        validate_spread_constraint(1.5)

    def test_rejects_negative(self) -> None:
        with pytest.raises(ValueError, match="Spread constraint violated"):
            validate_spread_constraint(-0.1)
...
class TestValidateIntensityConstraint:
    ...
    def test_accepts_zero(self) -> None:
        validate_intensity_constraint(0.0)

    def test_accepts_one(self) -> None:
        validate_intensity_constraint(1.0)

    def test_accepts_midpoint(self) -> None:
        validate_intensity_constraint(0.5)

    def test_rejects_negative(self) -> None:
        with pytest.raises(ValueError, match="Intensity constraint violated"):
            validate_intensity_constraint(-0.1)

    def test_rejects_above_one(self) -> None:
        with pytest.raises(ValueError, match="Intensity constraint violated"):
            validate_intensity_constraint(1.1)
...
class TestValidateMicroPriceBounds:
    ...
    def test_accepts_in_range(self) -> None:
        validate_micro_price_bounds(10.0, 20.0, 15.0)

    def test_accepts_at_bid(self) -> None:
        validate_micro_price_bounds(10.0, 20.0, 10.0)

    def test_accepts_at_ask(self) -> None:
        validate_micro_price_bounds(10.0, 20.0, 20.0)

    def test_rejects_below_bid(self) -> None:
        with pytest.raises(ValueError, match="Micro price constraint violated"):
            validate_micro_price_bounds(10.0, 20.0, 5.0)

    def test_rejects_above_ask(self) -> None:
        with pytest.raises(ValueError, match="Micro price constraint violated"):
            validate_micro_price_bounds(10.0, 20.0, 25.0)
...
class TestValidateAskReconstruction:
    ...
    def test_accepts_matching(self) -> None:
        ...
        validate_ask_reconstruction(15.0, 10.0, 0.5, 10.0)

    def test_rejects_mismatch(self) -> None:
        with pytest.raises(ValueError, match="Ask reconstruction constraint violated"):
            validate_ask_reconstruction(99.0, 10.0, 0.5, 10.0)
