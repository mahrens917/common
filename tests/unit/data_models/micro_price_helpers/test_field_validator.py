...
from common.data_models.micro_price_helpers.field_validator import FieldValidator
...
class TestValidateStrike:
    ...
    def test_accepts_positive(self) -> None:
        FieldValidator.validate_strike(100.0)

    def test_rejects_zero(self) -> None:
        with pytest.raises(TypeError, match="Strike price must be positive"):
            FieldValidator.validate_strike(0.0)

    def test_rejects_negative(self) -> None:
        with pytest.raises(TypeError, match="Strike price must be positive"):
            FieldValidator.validate_strike(-1.0)
...
class TestValidateForwardPrice:
    ...
    def test_accepts_positive(self) -> None:
        FieldValidator.validate_forward_price(100.0)

    def test_rejects_zero(self) -> None:
        with pytest.raises(ValueError, match="Forward price must be positive"):
            FieldValidator.validate_forward_price(0.0)
...
class TestValidateDiscountFactor:
    ...
    def test_accepts_positive(self) -> None:
        FieldValidator.validate_discount_factor(0.99)

    def test_rejects_zero(self) -> None:
        with pytest.raises(ValueError, match="Discount factor must be positive"):
            FieldValidator.validate_discount_factor(0.0)
