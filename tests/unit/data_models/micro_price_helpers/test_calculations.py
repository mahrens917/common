...
class TestComputeIntrinsicValue:
    ...
    def test_call_option_in_the_money(self) -> None:
        ...
        result = MicroPriceCalculator.compute_intrinsic_value(
            option_type="call", strike=100.0, spot_price=110.0
        )
        assert result == 10.0
    ...
    def test_call_option_at_the_money(self) -> None:
        ...
        result = MicroPriceCalculator.compute_intrinsic_value(
            option_type="call", strike=100.0, spot_price=100.0
        )
        assert result == 0.0
    ...
    def test_call_option_out_of_the_money(self) -> None:
        ...
        result = MicroPriceCalculator.compute_intrinsic_value(
            option_type="call", strike=100.0, spot_price=90.0
        )
        assert result == 0.0
    ...
    def test_put_option_in_the_money(self) -> None:
        ...
        result = MicroPriceCalculator.compute_intrinsic_value(
            option_type="put", strike=100.0, spot_price=90.0
        )
        assert result == 10.0
    ...
    def test_put_option_at_the_money(self) -> None:
        ...
        result = MicroPriceCalculator.compute_intrinsic_value(
            option_type="put", strike=100.0, spot_price=100.0
        )
        assert result == 0.0
    ...
    def test_put_option_out_of_the_money(self) -> None:
        ...
        result = MicroPriceCalculator.compute_intrinsic_value(
            option_type="put", strike=100.0, spot_price=110.0
        )
        assert result == 0.0
    ...
    def test_mixed_case_call(self) -> None:
        """Test that 'Call' is handled case-insensitively."""
        result = MicroPriceCalculator.compute_intrinsic_value(
            option_type="Call", strike=100.0, spot_price=105.0
        )
        assert result == 5.0
    ...
    def test_deep_in_the_money_call(self) -> None:
        ...
        result = MicroPriceCalculator.compute_intrinsic_value(
            option_type="call", strike=100.0, spot_price=200.0
        )
        assert result == 100.0
    ...
    def test_deep_in_the_money_put(self) -> None:
        ...
        result = MicroPriceCalculator.compute_intrinsic_value(
            option_type="put", strike=200.0, spot_price=100.0
        )
        assert result == 100.0
    ...
    def test_small_intrinsic_value(self) -> None:
        ...
        result = MicroPriceCalculator.compute_intrinsic_value(
            option_type="call", strike=100.0, spot_price=100.01
        )
...
class TestComputeTimeValue:
    ...
    def test_time_value_for_otm_option(self) -> None:
        ...
        result = MicroPriceCalculator.compute_time_value(
            option_type="call", strike=100.0, spot_price=90.0, micro_price=5.0
        )
    ...
    def test_time_value_for_itm_option(self) -> None:
        ...
        result = MicroPriceCalculator.compute_time_value(
            option_type="call", strike=100.0, spot_price=110.0, micro_price=15.0
        )
    ...
    def test_time_value_for_atm_option(self) -> None:
        ...
        result = MicroPriceCalculator.compute_time_value(
            option_type="call", strike=100.0, spot_price=100.0, micro_price=3.0
        )
        # Intrinsic = 0, so time value = micro_price
        assert result == 3.0
    ...
    def test_zero_time_value(self) -> None:
        ...
        result = MicroPriceCalculator.compute_time_value(
            option_type="call", strike=100.0, spot_price=110.0, micro_price=10.0
        )
    ...
    def test_negative_time_value_clamped_to_zero(self) -> None:
        ...
        result = MicroPriceCalculator.compute_time_value(
            option_type="call", strike=100.0, spot_price=110.0, micro_price=5.0
        )
    ...
    def test_put_option_time_value(self) -> None:
        ...
        result = MicroPriceCalculator.compute_time_value(
            option_type="put", strike=100.0, spot_price=90.0, micro_price=15.0
        )
        # Intrinsic = 10, so time value = 15 - 10 = 5
        assert result == 5.0
    ...
    def test_all_time_value_no_intrinsic(self) -> None:
        ...
        result = MicroPriceCalculator.compute_time_value(
            option_type="put", strike=100.0, spot_price=110.0, micro_price=2.5
        )
...
class TestComputeMicroPriceMetrics:
    ...
    def test_delegates_to_metrics_calculator(self, mock_compute: MagicMock) -> None:
        ...
        result = MicroPriceCalculator.compute_micro_price_metrics(
            best_bid=49.0, best_ask=51.0, bid_size=100.0, ask_size=200.0
        )

        mock_compute.assert_called_once_with(
            bid_price=49.0, ask_price=51.0, bid_size=100.0, ask_size=200.0, symbol="UNKNOWN"
        )
    ...
    def test_reorders_return_values(self, mock_compute: MagicMock) -> None:
        ...
        result = MicroPriceCalculator.compute_micro_price_metrics(
            best_bid=50.0, best_ask=52.0, bid_size=150.0, ask_size=100.0
        )
    ...
    def test_passes_symbol_unknown(self, mock_compute: MagicMock) -> None:
        ...
        MicroPriceCalculator.compute_micro_price_metrics(
            best_bid=49.0, best_ask=51.0, bid_size=100.0, ask_size=200.0
        )
    ...
    def test_maps_parameter_names(self, mock_compute: MagicMock) -> None:
        ...
        MicroPriceCalculator.compute_micro_price_metrics(
            best_bid=48.0, best_ask=52.0, bid_size=120.0, ask_size=80.0
        )
    ...
    def test_returns_six_element_tuple(self, mock_compute: MagicMock) -> None:
        ...
        result = MicroPriceCalculator.compute_micro_price_metrics(
            best_bid=50.0, best_ask=51.5, bid_size=100.0, ask_size=100.0
        )
    ...
    def test_handles_equal_bid_ask_sizes(self, mock_compute: MagicMock) -> None:
        ...
        result = MicroPriceCalculator.compute_micro_price_metrics(
            best_bid=49.0, best_ask=51.0, bid_size=100.0, ask_size=100.0
        )
    ...
    def test_handles_large_spread(self, mock_compute: MagicMock) -> None:
        ...
        result = MicroPriceCalculator.compute_micro_price_metrics(
            best_bid=50.0, best_ask=60.0, bid_size=100.0, ask_size=200.0
        )
